...
function formatGateway(gateway: string | null): string {
  if (!gateway) return '';
  if (gateway === 'manual') return 'Havale / EFT';
      ...
      <section className="py-8 sm:py-12">
        <Container>
          <div className="mx-auto max-w-xl text-center">
            ...
            <p className="text-sm text-text-muted">{t('missingParams')}</p>
          </div>
        </Container>
      </section>
    );
                  ...
                  {timelineSteps.map((step, i) => (
                    <div key={i} className="flex gap-3">
                      ...
                      <div className={`pb-4 ${i === timelineSteps.length - 1 ? 'pb-0' : ''}`}>
                        ...
                        {step.date && (
                          <p className="text-xs text-text-muted">{formatDate(step.date, locale)}</p>
                        )}
                      </div>
                    </div>
                  ))}
